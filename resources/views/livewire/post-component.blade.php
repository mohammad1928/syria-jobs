...
<div id="{{$post->id}}" class="card posts-card">
            <div class="card-title row pt-3 pl-3">
                ...
                <div class="btn-group dropright post-menu">
                    ...
                    <div class="dropdown-menu">
                        <button wire:click="save({{$post->id}})"  class="dropdown-item btn"><i class="fas fa-bookmark text-primary"></i>
                        ...
                        @endif
                        <a href="#"  class="dropdown-item"><i class="fas fa-user-shield text-primary"></i> Report</a>
                    </div>
                </div>
            </div>
            <hr>
            ...
            <div class="card-footer d-flex justify-content-between">
                <button wire:click="like({{$post->id}})" class="btn @if(Auth::user()->isLiked($post->id)) btn-primary @else btn-outline-primary  @endif" title="Like Post"><i class="fas fa-thumbs-up mr-1"></i>{{count($post->likers)}}</button>
                <button wire:click="assignPost({{$post->id}})" class="btn btn-primary" title="Comments" data-toggle="modal" data-target="#showCommentsModal"><i class="far fa-comment mt-1 mr-1"></i>@if(count($post->comments)>0) {{count($post->comments)}} @endif</button>
...
<div wire:ignore.self  class="modal fade mt-4" id="showCommentsModal" tabindex="-1" role="dialog" aria-labelledby="exampleModalLabel" aria-hidden="true">
                <div  class="modal-dialog modal-dialog-scrollable" role="document" >
                        ...
                        <div class="modal-header">
                                <h5 class="modal-title" id="exampleModalLabel">Comments @if(count($comments)>0) ({{ count($comments) }}) @endif</h5>
                        ...
                        <div class="modal-body">
                            @foreach($comments as $comment)
                                <div class="p-2 mt-1">
                                   <div class="row">
                                        ...
                                        <div class="col-md-10 rounded p-2" style="background:#f6f6f6">
                                            <h5 class="ml-2 text-primary mt-2">
                                                ...
                                                <div class="btn-group dropright post-menu">
                                                    ...
                                                    <div class="dropdown-menu">
                                                        @if($comment->user_id==Auth::id())
                                                            <button wire:click="editComment({{$comment->id}})" class="dropdown-item"><i class="fas fa-edit text-primary"></i> Edit Comment</button>
                                                            <button wire:click="deleteComment({{$comment->id}})" class="dropdown-item"><i class="fas fa-trash-alt text-danger"></i> Delete Comment</button>
                                                        @endif
                                                        <button wire:click="replyComment({{$comment->id}})" class="dropdown-item"><i class="fas fa-user-shield text-muted"></i> Reply Comment</button>
                                                    </div>
                                                </div>
                                            </h5>
                                ...
                                <div class="d-flex justify-content-between mt-2 ml-4 mb-3">
                                    ...
                                    <a class="">Like</a>
                                    <a wire:click="assignReply({{$comment->id}})" class="" style="cursor:pointer">Reply @if(count($comment->replies)>0) ({{count($comment->replies)}})@endif</a>
                                </div>
                                    @if($reply===$comment->id)
                                        <div class="ml-5 mt-1">
                                           @if(isset($allReplies))
                                           ...
                                           <div class="input-group mt-2 ml-3 mr-3">
                                                    <input wire:model="replyText" type="text" class="form-control" placeholder="Type a reply...">
                            ...
                            <form wire:submit.prevent="addComment" class="w-100">
                                <div class="form-group">
                                    <div class="input-group">
                                        <input wire:model="comment" type="text" class="form-control" placeholder="Type a comment...">
                                        <span class="input-group-prepend"><button type="submit" class="btn btn-primary" title="Send"><i class="fas fa-paper-plane"></i></button></span>
                                    </div>
                                </div>
                            </form>
